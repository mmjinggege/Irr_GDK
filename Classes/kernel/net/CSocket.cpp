...
bool CSocket::create(const char* pszServerIP, int nServerPort, int nBlockSec /* = BLOCKSECONDS */, bool bKeepAlive /* = false */)
{
	// 检查参数
...
bool CSocket::sendMsg(void* pBuf, int nSize)
{
	if(pBuf == 0 || nSize <= 0) 
		return false;
...
bool CSocket::receiveMsg(void* pBuf, int& nSize)
{
	//检查参数
...
bool CSocket::recvFromSock(void)
{
...
bool CSocket::Flush(void)		//? 如果 OUTBUF > SENDBUF 则需要多次SEND（）
{
	if (m_sockClient == INVALID_SOCKET) 
		return false;
