...
namespace irr_net
{
	...
	bool IrrSocket::create(const char* pszServerIP, int nServerPort, int nBlockSec /* = BLOCKSECONDS */, bool bKeepAlive /* = false */)
	{
		// 检查参数
	...
	bool IrrSocket::sendMsg(void* pBuf, int nSize)
	{
		if(pBuf == 0 || nSize <= 0) 
			return false;
	...
	bool IrrSocket::receiveMsg(void* pBuf, int& nSize)
	{
		//检查参数
	...
	bool IrrSocket::recvFromSock(void)
	{
	...
	bool IrrSocket::Flush(void)		//? 如果 OUTBUF > SENDBUF 则需要多次SEND（）
	{
		if (m_sockClient == INVALID_SOCKET) 
			return false;
